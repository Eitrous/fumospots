...
const CLUSTER_BUBBLE_STROKE_WIDTH_PX = 1;
...
const MARKER_COLLISION_GAP_PX = 0;
...
const desktopPreviewStyle = computed(() => {
  ...
  const maxHeight = Math.max(
    180,
    Math.min(height - DESKTOP_PREVIEW_MARGIN_PX * 2, 448),
  );
  const maxLeft = Math.max(
    DESKTOP_PREVIEW_MARGIN_PX,
    width - DESKTOP_PREVIEW_MARGIN_PX - DESKTOP_PREVIEW_WIDTH_PX,
  );
  const maxTop = Math.max(
    DESKTOP_PREVIEW_MARGIN_PX,
    height - DESKTOP_PREVIEW_MARGIN_PX - maxHeight,
  );
...
const getDisplayFeatureKey = (
  feature: GeoJSON.Feature<GeoJSON.Point, DisplayPointProperties>,
) => {
...
const getSelectedPostCoordinates = async (
  postId: number,
): Promise<[number, number] | null> => {
  const selectedFeature = collection.value.features.find(
...
const focusSelectedPost = async (postId: number) => {
  ...
  if (
    !coordinates ||
    currentSequence !== selectedPostFocusSequence ||
    !mapRef.value
  ) {
    emit("fly-completed");
    return;
  ...
  mapRef.value.flyTo({
    ...
    essential: true,
    ...(shouldUseOverviewFlight
      ? { minZoom: SELECTED_POST_FOCUS_OVERVIEW_ZOOM }
      : {}),
  });
};
...
const syncSelectionSource = () => {
  ...
  const source = mapRef.value.getSource(
    "selected-post",
...
const getClusterBreakoutPadding = () => {
  ...
  return {
    ...
    right: Math.max(
      basePadding.right,
      Math.round(mapEl.value.clientWidth * 0.15),
    ),
    bottom: Math.max(
      basePadding.bottom,
      Math.round(mapEl.value.clientHeight * 0.15),
    ),
    left: Math.max(
      basePadding.left,
      Math.round(mapEl.value.clientWidth * 0.15),
...
const getDisplayedMarkerOuterRadiusPx = (pointCount: number) => {
  ...
  return (
    getClusterBubbleFillRadiusPx(pointCount) + CLUSTER_BUBBLE_STROKE_WIDTH_PX
  );
};
...
const createCollisionNodeFromRawMembers = (
  rawMembers: ScreenPointMember[],
): CollisionNode => {
  ...
  return {
    adjustedLng:
      rawMembers.reduce((sum, member) => sum + member.adjustedLng, 0) /
      memberCount,
      ...
      for (let xOffset = -1; xOffset <= 1; xOffset += 1) {
        for (let yOffset = -1; yOffset <= 1; yOffset += 1) {
          const bucket = cells.get(
            `${cellXByIndex[currentIndex] + xOffset}:${cellYByIndex[currentIndex] + yOffset}`,
          );
          if (!bucket?.length) {
            continue;
          }
          ...
          for (const candidateIndex of bucket) {
            ...
            const collisionDistance =
              current.collisionRadiusPx + candidate.collisionRadiusPx + gapPx;
...
const mergeCollisionNodeGroup = (nodes: CollisionNode[]) => {
  return createCollisionNodeFromRawMembers(
    nodes.flatMap((node) => node.rawMembers),
  );
};

const resolveDisplayCollisionGroups = (rawMembers: ScreenPointMember[]) => {
  return rawMembers.map((member) => [member]);
};
// const resolveDisplayCollisionGroups = (rawMembers: ScreenPointMember[]) => {
...
const resolveMaxZoomCollisionGroups = (rawMembers: ScreenPointMember[]) => {
  if (!rawMembers.length) {
    return [] as ScreenPointMember[][];
  }
  ...
  const scaledMembers = scaleScreenPointMembers(rawMembers, scaleAtMaxZoom);

  const nodes = scaledMembers.map((member) => ({
    adjustedLng: member.adjustedLng,
    collisionRadiusPx: POINT_MARKER_OUTER_RADIUS_PX,
    lat: member.lat,
    rawMembers: [member],
  ...
  return clusterCollisionNodes(nodes, MARKER_COLLISION_GAP_PX).map((group) =>
    group.flatMap((node) => node.rawMembers),
  );
};

const scaleScreenPointMembers = (
  members: ScreenPointMember[],
  scale: number,
) => {
...
const buildClusterState = (
  ...
  const sortedIds = members
    ...
    .sort((left, right) => left - right);
  const key = sortedIds.join(":");
  const centerAdjustedLng =
    members.reduce((sum, member) => sum + member.adjustedLng, 0) /
    members.length;
  const centerLat =
  ...
  const minAdjustedLng = Math.min(
    ...members.map((member) => member.adjustedLng),
  );
  const maxAdjustedLng = Math.max(
    ...members.map((member) => member.adjustedLng),
  );
  const minLat = Math.min(...members.map((member) => member.lat));
  const maxLat = Math.max(...members.map((member) => member.lat));
...
const syncDisplaySource = () => {
  ...
  const nextPreviewCluster = displayClusterStateByKey.get(
    activePreviewGroupKey.value,
  );
...
const ensureRegionHighlightLayers = () => {
  ...
  const fillColor = isDark.value
    ...
    : "rgba(22, 146, 95, 0.1)";
  const outlineColor = isDark.value
    ? "rgba(88, 199, 143, 0.78)"
    : "rgba(22, 146, 95, 0.68)";
...
const handleMapClick = async (event: MapMouseEvent) => {
  ...
  if (clusterFeature) {
    const clusterState = displayClusterStateByKey.get(
      getClusterGroupId(
        clusterFeature.properties as Record<string, unknown> | undefined,
      ),
    );
...
function scheduleBaseMapRecovery() {
  ...
  const delay =
    BASE_MAP_RECOVERY_RETRY_DELAYS_MS[
      Math.min(
        baseMapRecoveryAttempts,
        BASE_MAP_RECOVERY_RETRY_DELAYS_MS.length - 1,
      )
    ];
...
const loadRegionHighlight = async (scope: RegionScope | null) => {
  ...
  try {
    ...
    regionHighlightCollection.value = buildRegionHighlightCollection(
      scopeKey,
...
const teardownPreviewSheetPointerListeners = () => {
  ...
  window.removeEventListener(
    "pointermove",
    handlePreviewSheetWindowPointerMove,
  );
  window.removeEventListener("pointerup", handlePreviewSheetWindowPointerUp);
  window.removeEventListener(
    "pointercancel",
    handlePreviewSheetWindowPointerCancel,
  );
...
function handlePreviewSheetWindowPointerMove(event: PointerEvent) {
  ...
  previewSheetDragOffset.value = Math.max(
    0,
    event.clientY - previewSheetPointerStartY,
  );
}
...
function handlePreviewSheetWindowPointerUp(event: PointerEvent) {
  ...
  const shouldClose =
    previewSheetDragOffset.value >= PREVIEW_SHEET_CLOSE_THRESHOLD_PX;
  previewSheetDragging.value = false;
  previewSheetDragOffset.value = 0;
  teardownPreviewSheetPointerListeners();
...
<template>
  <div class="map-stage">
    ...
    <div
      ...
      @click.stop
    >
      <div class="map-preview-list" :class="{ 'is-loading': previewLoading }">
        <p v-if="previewLoading" class="map-preview-state">
          {{ t("map.previewLoading") }}
        </p>
        <p v-else-if="previewError" class="map-preview-state">
          {{ previewError }}
        </p>
        <p v-else-if="!previewItems.length" class="map-preview-state">
          {{ t("map.previewEmpty") }}
        </p>
        <template v-else>
          <button
          ...
          >
            ...
            <span class="map-preview-item__title">{{
              item.title || t("map.untitledPost")
            }}</span>
          </button>
        </template>
      </div>
    ...
    <Transition name="map-preview-sheet">
      ...
      >
        <div
          ...
          :class="[
            previewSurfaceClass,
            { 'is-dragging': previewSheetDragging },
          ]"
...
.map-preview-sheet {
  ...
  color: var(--ink);
  opacity: 1;
  transform: translateY(var(--preview-sheet-enter-offset))
    translateY(var(--preview-sheet-drag-offset));
  transition:
